...
public abstract class Node extends Object {
  ...
  protected Node(Directory parent, String name) throws FSException {
    this.name = name;
    this.parent = parent;
  ...
  public boolean move(IDirectory dst, String name) throws FSException {
	  ...
	  if (dst == null || dst.name()==null || dst == this) {
		  throw new FSException("the destination isn't correct");
	  }
	  if (name == null || name == "") {
		  throw new FSException("The name isn't correct");
	  }
	  Directory direct = (Directory)dst;
	  if (direct.dir(name)==null && direct.file(name)==null) {
		  ((Directory)this.parent).remove(this);
		  this.parent = direct;
		  this.name = name;
		  ((Directory)direct).add(this);
		  return true;
	  }
	  return false;  }
